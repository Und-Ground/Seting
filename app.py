...
# Add these new functions before run_ffmpeg
def download_video_ytdlp(url, output_dir="downloads/videos"):
...
def get_random_file(folder_path, extensions=('.mp4', '.mkv', '.avi', '.mov')):
    """Get a random file from folder with given extensions"""
...
def run_ffmpeg_enhanced(main_video, random_video_folder, stream_key, is_shorts, log_callback, 
                        audio_file=None, random_audio_folder=None, rtmp_url=None, session_id=None):
    """Run FFmpeg with support for main video followed by random videos and custom audio"""
    ...
    while True:
        ...
        if not current_video or not os.path.exists(current_video):
            current_video = get_random_file(random_video_folder)
            if not current_video:
                log_callback("❌ No video files found in random folder. Stopping.")
                break
        
        log_callback(f"📺 Now streaming: {os.path.basename(current_video)}")
        ...
        elif random_audio_folder and os.path.exists(random_audio_folder):
            rand_audio = get_random_file(random_audio_folder, ('.mp3', '.m4a', '.wav'))
            if rand_audio:
                audio_input = ["-i", rand_audio]
                audio_mapping = ["-map", "0:v", "-map", "1:a"]
            else:
                audio_mapping = ["-map", "0:v", "-map", "0:a"]
        else:
            audio_mapping = ["-map", "0:v", "-map", "0:a"]

        cmd = [
            "ffmpeg", "-re", "-i", current_video
        ] + audio_input + [
        ...
        time.sleep(1) # Small delay between video switches
...
def main():
    ...
    with col1:
        ...
        st.markdown("### 📥 Video Downloader & Manager")
        
        tab1, tab2, tab3 = st.tabs(["Single URL", "Bulk URL List", "Manual Upload"])
        
        with tab1:
            dl_url = st.text_input("YouTube/Video URL to Download", key="dl_single")
            target_folder = st.selectbox("Target Folder", ["downloads/videos", "downloads/random_videos"], key="folder_single")
            if st.button("Download Now", key="btn_single"):
                if dl_url:
                    with st.status("Downloading...", expanded=True) as status:
                        success, result = download_video_ytdlp(dl_url, output_dir=target_folder)
                        if success:
                            status.update(label=f"✅ Downloaded to {target_folder}", state="complete")
                            st.success(f"Saved: {os.path.basename(result)}")
                        else:
        ...
        with tab2:
            bulk_urls = st.text_area("List of URLs (one per line)", height=150, key="dl_bulk")
            target_folder_bulk = st.selectbox("Target Folder", ["downloads/videos", "downloads/random_videos"], key="folder_bulk")
            if st.button("Download All", key="btn_bulk"):
                urls = [u.strip() for u in bulk_urls.split("\n") if u.strip()]
                if urls:
                    progress_bar = st.progress(0)
                    for i, url in enumerate(urls):
                        with st.status(f"Downloading ({i+1}/{len(urls)}): {url}", expanded=False):
                            success, result = download_video_ytdlp(url, output_dir=target_folder_bulk)
                            if success:
                                st.write(f"✅ Success: {os.path.basename(result)}")
                            else:
                                st.write(f"❌ Failed: {url} - {result}")
                        progress_bar.progress((i + 1) / len(urls))
                    st.success("Bulk download process completed!")
                else:
                    st.warning("Please enter at least one URL")

        with tab3:
            uploaded_videos = st.file_uploader("Upload Video Files", type=['mp4', 'mkv', 'avi', 'mov', 'flv'], accept_multiple_files=True)
            target_folder_up = st.selectbox("Target Folder", ["downloads/videos", "downloads/random_videos"], key="folder_upload")
            if st.button("Save Uploaded Files"):
                if uploaded_videos:
                    os.makedirs(target_folder_up, exist_ok=True)
                    for uploaded_file in uploaded_videos:
                        file_path = os.path.join(target_folder_up, uploaded_file.name)
                        with open(file_path, "wb") as f:
                            f.write(uploaded_file.read())
                        st.write(f"✅ Saved: {uploaded_file.name}")
                    st.success(f"All files saved to {target_folder_up}")
                else:
        ...
        stream_mode = st.radio("Streaming Mode", ["Single Video", "Main Video + Random Folder", "Random Folder Only"])
        
        sm_col1, sm_col2 = st.columns(2)
        
        with sm_col1:
            main_video_path = None
            if stream_mode in ["Single Video", "Main Video + Random Folder"]:
                # Get available videos
                available_videos = []
                for root, dirs, files in os.walk('downloads/videos'):
                    for file in files:
                        if file.lower().endswith(('.mp4', '.mkv', '.avi', '.mov')):
                            available_videos.append(os.path.join(root, file))
                ...
                if available_videos:
                    main_video_path = st.selectbox("Select Main Video", available_videos)
                else:
                    main_video_path = st.text_input("Main Video Path", value="downloads/videos/")
                
            random_video_folder = None
            if stream_mode in ["Main Video + Random Folder", "Random Folder Only"]:
                random_video_folder = st.text_input("Random Video Folder", value="downloads/random_videos/")

        with sm_col2:
            audio_mode = st.radio("Audio Mode", ["Video Original", "Single Audio Loop", "Random Audio Folder"])
            audio_file_path = None
            random_audio_folder_path = None
            
            if audio_mode == "Single Audio Loop":
                audio_file_path = st.text_input("Audio File Path", value="downloads/audio/")
            elif audio_mode == "Random Audio Folder":
                random_audio_folder_path = st.text_input("Random Audio Folder", value="downloads/random_audio/")

        is_shorts_mode = st.checkbox("Vertical Mode (Shorts/TikTok)", value=False)

        if st.button("🚀 Start Streaming", type="primary"):
            current_sk = st.session_state.get('current_stream_key')
            if not current_sk:
                st.error("Please provide or generate a stream key first")
            else:
                session_id = datetime.now().strftime("%Y%m%d_%H%M%S")
                st.session_state['streaming_active'] = True
                
                # Start the enhanced streaming thread
                st.info(f"Starting stream session: {session_id}")
                st.write("Logs will be available in the sidebar/database.")
                
                thread = threading.Thread(
                    target=run_ffmpeg_enhanced,
                    args=(main_video_path, random_video_folder, current_sk, is_shorts_mode, st.write, 
                          audio_file_path, random_audio_folder_path, None, session_id)
                )
                thread.daemon = True
                thread.start()
